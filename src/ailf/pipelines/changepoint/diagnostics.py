"""Deterministic, training-only diagnostics (FR-013/014).

Every field is computed from the training frame alone — no test data, no audit_only info. The FULL
13-field bundle is always computed (so intra-bundle dependencies hold), but ``to_agent_dict(enabled)``
returns only the configured-enabled fields — disabled diagnostics are computed-but-hidden from the
agent (FR-013, research Decision 12). With every field enabled the filtered view is byte-identical
to the POC's ``asdict`` output (parity guard, SC-001).
"""
...
from ailf.pipelines.changepoint.detector import ChangepointSet, detect_changepoints
...
class DiagnosticsBundle:
    ...
    def to_agent_dict(self, enabled: Iterable[str] | None = None) -> dict[str, Any]:
        """Agent-facing view, filtered to ``enabled`` field names (FR-013, Decision 12).

        The full bundle is always computed; this returns only the enabled subset shown to the
        decision node. ``enabled=None`` returns ALL fields (the golden/all-enabled view), which is
        byte-identical to the POC's ``asdict`` output (parity guard). Unknown names in ``enabled``
        are ignored; the field order matches the dataclass declaration for stable output.
        """
        full = asdict(self)
        if enabled is None:
            return full
        enabled_set = set(enabled)
        return {k: v for k, v in full.items() if k in enabled_set}

    @staticmethod
    def field_names() -> tuple[str, ...]:
        """The 13 bundle field names — the single source of truth for config lockstep (FR-011)."""
        return tuple(f.name for f in fields(DiagnosticsBundle))

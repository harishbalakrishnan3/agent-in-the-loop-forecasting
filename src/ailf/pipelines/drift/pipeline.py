"""Drift dataset-generation API.

Wires the DriftGenerator into a FastAPI application that exposes a Swagger UI for runtime control
of synthetic drift dataset generation.

The forecast/changepoint UI surfaces that previously lived here (the embedded HTML forecast page,
the ``/forecast/*`` endpoints, and the ``/changepoint/run`` bridge) were removed in feature 006:
the single presentation-facing interface is now ``src/ailf/ui/app.py`` (run with
``uv run streamlit run src/ailf/ui/app.py``). This module is now solely the drift dataset service.
...
def main() -> None:
    """Launch the drift dataset-generation API server."""
    uvicorn.run(

...
the directory to sys.path so same-dir imports work. Run with:
    ...
    uv run pytest pocs/changepoint/seasonalityV2/ref_test_seasonal.py
...
from ref_datasets_seasonal import generate_seasonal_shift, generate_control
...
def test_working_pen_exists():
    ...
    noted as a comment in ref_tools_seasonal.py (POC TUNING RESULTS block).
    """
    series, true_break = generate_seasonal_shift(seed=42)
    control = generate_control(seed=99)
    ...
    # Print working range for the developer to pin in ref_tools_seasonal.py
